...
volatile int avg_count_spO2 = 0;
volatile double max_IR = 0;
volatile double min_IR = 0;
volatile double max_R = 0;
volatile double min_R = 0;
// Constructor
spO2Readout::spO2Readout(int coord_x, int coord_y, int width, int len, int pin_R, int pin_IR, int pin_detector_out, int pin_detector_in, int reset_timer, Adafruit_ILI9341* tft):coord_x(coord_x), coord_y(coord_y), len(len), width(width), pin_detector_in(pin_detector_in), pin_detector_out(pin_detector_out), pin_IR(pin_IR), pin_R(pin_R), reset_timer(reset_timer), tft_interface(tft) {
...
void spO2Readout::draw(){
    tft_interface->drawRect(this->coord_x,this->coord_y,this->len,this->width, ILI9341_BLACK);
	// Turn on LEDs for SpO2 probe
	pinMode(pin_IR, OUTPUT);
	pinMode(pin_detector_out, OUTPUT);
	pinMode(pin_R, OUTPUT);
	pinMode(pin_detector_in, INPUT);
	digitalWrite(pin_detector_out, HIGH);
...
void spO2Readout::read(){
    ...
    if (avg_count_spO2 < 5) {
        double input = (double) analogRead(pin_detector_in);
        double adjusted = (input /1023);
        // Put number in next location in fifo
        averager_queue[avg_count_spO2] = adjusted;
        avg_count_spO2 += 1;
...
Vector<double>& spO2Readout::getFifo(void) {
	Vector<double> dump(fifo);
	return dump;
}

void spO2Readout::display_signal(){
